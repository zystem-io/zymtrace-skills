...
1. **Rank first if the request is rank-shaped** ("what's allocating the most", "which code creates the most garbage") — use **`discover`** / **`top_functions`** on 26.8.1+ (**`topentities`** / **`topfunctions`** on older instances) with event kind **`alloc`**, then drill into the top site with `hot_traces`. Rank by **bytes allocated** over the window (the `alloc` kind's weight is bytes; show the figure per entry); include host and container names where the data has them. Mark unmodifiable runtime/library allocations (JIT, classloading, framework internals you can't change) non-actionable and drill into the highest user-owned site. See scope-to-own-code/ROI in the shared doc.
...
3. **Pull the allocation call tree** at the named scope (executable / container / pod / host / time range), event kind **`alloc`** — use the **`hot_traces`** MCP tool when available (zymtrace 26.5.1+), else fall back to **`flamegraph`** (see the data-source policy in the shared doc). Check `recommendations` first (both generations, wide range, e.g. 30 days), then on 26.8.1+ **survey then drill** `hot_traces`: default request, then the top 1–3 traces in full (`prefix_hash` verbatim, `full_traces = true`, page size 1); before analyzing full traces, fetch the `hot_trace_examine` prompt (or the `prompt://hot_trace?op=examine` resource) — there's no allocation-specific prompt, but its memory/GC heuristics are the rubric the backend's `recommendations` were generated under (see "Analysis prompts" in the shared doc). Read which allocation sites dominate by bytes — and never compare `alloc` bytes against `on_cpu` cores; the kinds are incomparable.
