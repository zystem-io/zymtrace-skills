...
1. **Rank first if the request is rank-shaped** ("what's eating my CPU", "which process", "biggest ROI") — use **topentities** / **topfunctions**. **Rank by cores consumed** (CPU-cores, i.e. the absolute on-CPU time the entity holds — not just % of one core), so the ranking reflects real machine cost; show the cores figure **and its annualized cost** for each entry (see [Cost: annualize the cores](#cost-annualize-the-cores)). **Where the data has them, include the host name and the container name** alongside each ranked entry (e.g. `inference-api · host node-7 · container api-7f9c`) so the user can locate the workload — omit a field only when the MCP doesn't return it. Then pick the top entry. Mark unmodifiable system processes (kube-proxy, kubelet, systemd, the kernel) non-actionable and drill into the highest user-owned entry. See scope-to-own-code/ROI in the shared doc.
...
- **Annual cost ≈ cores × per-vCPU-hour rate × 8,760 h** (= cores × per-vCPU-year rate).
...
- [ ] Ranked first when the request was rank-shaped; system/third-party processes marked non-actionable, the top user-owned entry drilled into.
- [ ] Cores shown over the analysis window **and** annualized to a yearly cost; the fix's saving dollarized in Expected Impact.
